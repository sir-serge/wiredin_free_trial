...
def get_player():
    ...
    index = 0   
    # Enumerate the hand options to assign an index to each and build the prompt message
    for index, hand in enumerate(hands):        # Build the input message with hand options
        input_message += f"{str(index)} :{hand}" 
        if index < 2:         
            input_message += ', '
        index += 1  
